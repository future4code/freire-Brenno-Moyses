...
class Migrations extends BaseDatabase {
    ...
    createTables = async () => {
        ...
        DROP TABLE IF EXISTS ${OrderDatabase.TABLE_ORDERS_ITEMS};
        ...
        CREATE TABLE IF NOT EXISTS ${OrderDatabase.TABLE_ORDERS_ITEMS} (
            id VARCHAR(255) PRIMARY KEY,
            pizza_name VARCHAR(255) NOT NULL,
            quantity TINYINT,
            order_id VARCHAR(255) NOT NULL,
            FOREIGN KEY (pizza_name) REFERENCES Ambulnz_Pizzas (name),
            FOREIGN KEY (order_id) REFERENCES Ambulnz_Orders (id)
        );
        `)
    }
